...
+ (StoreManager *)sharedInstance
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        storeManagerSharedInstance = [[StoreManager alloc] initWithProductIdentifiers:productIdentifiers];
    });
...
- (instancetype)initWithProductIdentifiers:(NSSet *)productIdentifiers
{
    ...
    if (self != nil) {
        ...
        _productIdentifiers = [[NSSet alloc] initWithSet:productIdentifiers];
    }
    return self;
}
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response
{
    ...
    if ((response.products).count > 0) {
        model = [[SMModel alloc] initWithName:@"AVAILABLE PRODUCTS" elements:response.products];
    ...
    if ((response.invalidProductIdentifiers).count > 0) {
        model = [[SMModel alloc] initWithName:@"INVALID PRODUCT IDS" elements:response.invalidProductIdentifiers];
        [self.productRequestResponse addObject:model];
    }
